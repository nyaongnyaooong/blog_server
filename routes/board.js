...
// 게시판 게시글 리스트 api
router.get("/board/data", (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    `);
        const [resSQL2] = yield mySQL.query(`SELECT board_serial, COUNT(*) AS comments
      FROM comment 
...
// 게시글 생성 라우터
router.post("/board", (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const mySQL = yield database_1.mySQLPool.getConnection();
    try {
        const { title: reqPostTitle, content: reqPostContent } = req.body;
        // body 데이터가 정상적으로 들어오지 않거나 값이 없음
        if (typeof reqPostTitle !== 'string' || reqPostTitle === '')
            throw new CustomError('제목을 입력하지 않았습니다');
        if (typeof reqPostContent !== 'string' || reqPostContent === '')
            throw new CustomError('내용을 입력하지 않았습니다');
        ...
        yield mySQL.beginTransaction();
        const [result] = yield mySQL.query(`INSERT INTO board(user_serial, user_id, title, content, date)
    VALUES(${userSerial}, '${userID}', '${reqPostTitle}', '${reqPostContent}', NOW())`);
        yield mySQL.commit();
        // DB 추가에 실패함
        if (!('affectedRows' in result) || !result.affectedRows)
            throw new CustomError('DB 업데이트에 실패했습니다');
        res.send({
            result: true,
            error: false
        });
...
router.patch("/board/:serial", (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    WHERE board_serial=${reqPostSerial}`);
        ...
        if (userSerial !== user_serial)
            throw new CustomError('05');
        // 게시물 수정
        const [resSQL2] = yield mySQL.query(`UPDATE board
...
// 게시글 읽기 페이지
router.get('/board/:serial', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const mySQL = yield database_1.mySQLPool.getConnection();
    const { serial: reqPostSerial } = req.params;
    try {
        const { serial: userSerial, id: userID } = req.user;
      ...
      FROM board 
      WHERE board_serial=${reqPostSerial}`);
        const [resSQL2] = yield mySQL.query(`
      SELECT * 
      FROM comment 
      WHERE board_serial=${reqPostSerial}`);
        // 게시물이 존재하지 않음
        if (!resSQL1[0])
            throw new CustomError('게시물이 존재하지 않습니다');
...
// 게시물 삭제
router.delete('/board/:serial', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const mySQL = yield database_1.mySQLPool.getConnection();
    const { serial: reqPostSerial } = req.params;
    try {
        const { serial: userSerial, id: userID } = req.user;
    ...
    WHERE board_serial=${reqPostSerial}`);
        // 게시물이 존재하지 않음
        if (!resSQL1[0])
            throw new CustomError('존재하지 않는 게시물입니다');
        const { user_serial } = resSQL1[0];
        // 게시물 작성자와 로그인한 유저가 불일치
        if (userSerial !== user_serial && userID !== 'admin')
            throw new CustomError('해당 게시물의 작성자가 아닙니다');
    ...
    `);
        ...
        if (err instanceof CustomError) {
            if (err.message === '로그인 정보가 없습니다')
                statusCode = 401;
            if (err.message === '해당 게시물의 작성자가 아닙니다')
                statusCode = 403;
            errMessage = err.message;
        }
        else {
...
router.post('/comment', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        if (typeof reqContent !== 'string' || reqContent === '')
            throw new CustomError('내용을 입력해주세요');
        if (isNaN(reqCommentType))
            throw new CustomError('잘못된 요청입니다');
        const { serial: userSerial, id: userID } = req.user;
        // 유저 검증 미들웨어 문제
        if (!req.user || !userID)
        ...
        if (!userSerial || userID === 'anonymous')
            throw new CustomError('로그인 정보가 없습니다');
        yield mySQL.beginTransaction();
        const [resSQL] = yield mySQL.query(`INSERT INTO comment(user_serial, user_id, board_serial, content, date, reply)
...
// 댓글 수정 api
router.patch('/comment', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const mySQL = yield database_1.mySQLPool.getConnection();
    try {
        const { reqContent } = req.body;
        const reqSerial = Number(req.body.reqSerial);
        // body 데이터가 정상적으로 들어오지 않음
        if (isNaN(reqSerial) || typeof reqContent !== 'string' || reqContent === '')
            throw new CustomError('01');
        const { serial: userSerial, id: userID } = req.user;
        // 유저 검증 미들웨어 문제
        if (!req.user || !userID)
            throw new Error('유저 검증에 문제가 있습니다');
      ...
      WHERE comment_serial=${reqSerial}`);
        ...
        if (userSerial !== user_serial && userID !== 'admin')
            throw new CustomError('수정 권한이 없습니다');
        yield mySQL.query(`UPDATE comment
...
// 댓글 삭제 api 리팩토링
router.delete('/comment/:serial', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const mySQL = yield database_1.mySQLPool.getConnection();
    try {
        // 올바르지않은 path
        if (!req.params.serial || isNaN(Number(req.params.serial)) || isNaN(Number(req.body.boardSerial)))
            throw new CustomError('요청이 잘못되었습니다');
        ...
        if (!userSerial || userID === 'anonymous')
            throw new CustomError('로그인 정보가 없습니다');
        yield mySQL.beginTransaction();
        const [comments] = yield mySQL.query(`SELECT *
      FROM comment
      WHERE board_serial=${reqBoardSerial}`);
        ...
        if (reqComment.user_serial !== userSerial && userID !== 'admin')
            throw new CustomError('삭제 권한이 없습니다');
        // 대댓글인지확인
        // 대댓글이면
        if (reqComment.reply) {
            //삭제
            const [resSQL] = yield mySQL.query(`
        DELETE FROM comment 
        WHERE comment_serial=${reqCommentSerial}`);
            ...
            const replys = [];
            comments.forEach(e => {
                if (e.reply === reqComment.reply)
                    replys.push(e);
            });
            // 상위 댓글확인
            const upperCommentArr = [];
            comments.forEach(e => {
                if (e.comment_serial === reqComment.reply)
                    upperCommentArr.push(e);
            });
          ...
          WHERE comment_serial=${upperComment.comment_serial}`);
            ...
            const replys = [];
            for (let i = 0; i < comments.length; i++) {
                if (comments[i].reply === reqCommentSerial)
