...
router.get('/board/:serial', async (req, res) => {
  ...
  try {
    ...
    if (!view[reqPostSerial]) {
      ...
      const [resSQL3] = await mySQL.query<CommentSQLTable[]>(`UPDATE board
      SET view=view+1
      WHERE board_serial=${reqPostSerial}`);
...
router.post('/comment', async (req, res) => {
  ...
  try {
    ...
    if (isNaN(reqSerial)) throw new CustomError('01');
    if (typeof reqContent !== 'string' || reqContent === '') throw new CustomError('01');
    if (isNaN(reqCommentType)) throw new CustomError('01');
...
router.patch('/comment', async (req, res) => {
  ...
  try {
    ...
    if (userSerial !== user_serial && userID !== 'admin') throw new CustomError('05');
...
router.delete('/comment/:serial', async (req, res) => {
  ...
  try {
    ...
    // 댓글이 존재하지 않음
    if (!comments.length) throw new CustomError('04');
    ...
    if (reqComment.user_serial !== userSerial && userID !== 'admin') throw new CustomError('권한이 없습니다');
